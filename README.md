...
![plugin-manager plugin screenshot](./plugin-manager-screenshot.png)
...
| :-- | :-- | :-- | :-- |
| `basePath`    | string (**optional**)            | A base path on which the plugin will be mounted. Defaults to [the manifest name](https://github.com/Linkurious/lke-plugin-plugin-manager/blob/master/manifest.json#L2). | `"table"` |
...
Configuration keys supported only by the plugin-manager plugin:
...
Configuration example for 1 plugin-manager instance accesible via `/plugins/table`:
...
Configuration example for 2 plugin-manager instances accesible via `/plugins/table` and `/plugins/edgeTable`:
...
*You can run multiple plugin-manager instances by passing an array with unique `basePath` per config.*
...
3. Compose a valid plugin-manager plugin URL and open it in a new tab. For example:
```
